...
public class AuthController {
    ...
    // 로그인
    @Operation(summary = "로그인", description = "사용자의 로그인 아이디와 비밀번호를 입력받아 로그인")
    @PostMapping("/login")
    public CommonResponse<?> loginUser(@RequestBody UserDto.LoginRequestDto requestDto) {
        try {
            ...
            return CommonResponse.success("로그인 성공", responseDto);
        } catch (Exception e) { // 구현 시 별도의 예외 처리 필요, 일단 Exception으로 처리
            log.error("로그인 실패: {}", e.getMessage());
            return CommonResponse.fail(HttpStatus.FORBIDDEN,"로그인 실패");
        }
    }
    ...
    public CommonResponse<?> registerUser(@RequestBody UserDto.RegisterRequestDto requestDto) {
        try {
            ...
            return CommonResponse.success("회원가입 성공", responseDto);
        } catch (Exception e) { // 구현 시 별도의 예외 처리 필요, 일단 Exception으로 처리
            log.error("회원가입 실패: {}", e.getMessage());
            return CommonResponse.fail(HttpStatus.FORBIDDEN,"회원가입 실패");
        }
    }
}
